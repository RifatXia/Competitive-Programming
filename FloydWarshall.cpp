//Floyd Warshall is used to find all pair shortest path, to process the queries in O(1) time
//It does not work on negative cycles
//Test - https://vjudge.net/contest/550961#problem/C
//RifatXia
    ...
    while(cin >> n >> s >> q && n != 0 && s != 0 && q != 0)
    {
        ...
        for(int via = 1; via <= n; via++)
        {
            for(int i = 1; i <= n; i++)
            {
                for(int j = 1; j <= n; j++)
                {
                    ll num = max(cost[i][via], cost[via][j]);
                    cost[i][j] = min(cost[i][j], num);
                }
            }
        }
